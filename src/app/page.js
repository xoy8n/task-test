...
export default function Home() {
  ...
  const getAiResponse = useCallback(async (startLetter, usedWordsList) => {
    ...
    try {
      ...
      const prompt = `끝말잇기 게임입니다. 당신은 다음 단어를 추천해야 합니다. 조건: '${startLetter}'(으)로 시작해야 하고, [${usedWordsList.join(", ")}] 목록에 있는 단어는 사용하면 안 됩니다. 한 글자 단어도 안 됩니다. 다른 설명이나 말 없이, 오직 추천하는 한글 명사 단어 하나만을 텍스트로 응답해주세요. 예를 들어 "사과" 또는 "나무" 처럼 단어만 말하세요. 함수를 호출할 필요 없습니다.`;
